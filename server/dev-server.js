// Custom Next.js + Socket.io server (ASYNC pipeline, 5 chains max, security hardened)
...
const RATE_LIMIT = 15;      // events/sec per socket
...
app.prepare().then(() => {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('teacher:sendDemand', gate(socket, ({ chain, value }, cb) => {
      const state = gameByTeacher(socket.id);
      if (!state) return cb && cb({ ok: false, error: 'Không tìm thấy game' });
    ...
    socket.on('teacher:setCaptain', gate(socket, ({ chain, role, playerId }) => {
      ...
      const r = c[role];
      if (!r) return;
      r.players.forEach(p => (p.isCaptain = p.id === playerId));
      r.captainId = playerId;
      broadcastState(state.roomCode);
    }));
    ...
    socket.on('disconnect', () => {
      ...
      const state = roomCode && games.get(roomCode);
      if (state) {
        engine.removePlayer(state, socket.id);
        broadcastState(roomCode);
      }
    });
  });

  httpServer.listen(port, () => console.log(`> Beer Game ASYNC dev ready at http://localhost:${port}`));
});
